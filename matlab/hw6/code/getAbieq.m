function [Aieq, bieq] = getAbieq(n_seg, n_order, corridor_range, ts, v_max, a_max, j_max)
    ...
    for k = 1:n_seg
        for n = 1:n_ctr
            ...
            Aieq_v(index_row, (k-1)*(n_order+1)+n:(k-1)*(n_order+1)+n+1) = n_order * [-1, 1] * ts(k)^(0);
        end
    end
...
%     bieq_v = ones(n_eq/2,1)* v_max;
    ...
    for k = 1:n_seg
        for n = 1:n_ctr
            ...
            index_row = n+(k-1)*n_ctr;
            Aieq_a(index_row,(k-1)*(n_order+1)+n:(k-1)*(n_order+1)+n+2) = n_order * (n_order-1) * [1, -2, 1] * ts(k)^(-1);
        end
    end
